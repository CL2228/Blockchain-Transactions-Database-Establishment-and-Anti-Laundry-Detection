"""@Author: Chenghui Li, SCUT. Started on Jan 23, 2021.
    ...
    [Chain.csv]  CHAIN   Relationship
        :START_ID(Block)    ParentID    SonID   :END_ID(Block)
    ...
    [Tx2Block.csv]  Tx2Block    Relationship
        :START_ID(Tx)   TxID    BlockID     :END_ID(Block)

    [TxIn.csv]  TxIn    Node
        TxInID:ID(Tx)   TxID        Value       AddID
    ...
    [TxOut.csv]  TxOut    Node
        TxOutID:ID(TxOut)   TxID        AddID       Value
...
def block_chain_csv_process(year_path):
    ...
    chain_writer.writerow([':START_ID(Block)','ParentID', 'SonID', ':END_ID(Block)'])
    ...
    for i in tqdm.tqdm(range(1, len(blocks))):
        ...
        chain_writer.writerow([int(block[0]), int(block[0]) - 1, int(block[0]), int(block[0]) - 1])
    block_data.close()
...
def address_csv_process(year_path):
    ...
    address_writer.writerow(['AddID:ID(Address)', 'Address', 'Mixer'])
...
def annual_process(year_path):
    ...
    print("Transactions processing done, now starting processing Txin...")
    txin_csv_process(year_path)
    print("Txin processing done, now starting processing TxOut...")
    txout_csv_process(year_path)
